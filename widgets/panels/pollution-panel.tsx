...
export function PollutionPanel() {
  ...
  const structure = pollution.structure.map((s, i) => ({
    name: locale === "ru" ? s.name_ru : s.name_kk,
    value: s.percent,
    fill: SERIES[i % SERIES.length],
  }));

  const purity = pollution.purity_index.map((p) => ({
    name: locale === "ru" ? p.name_ru : p.name_kk,
    value: p.value,
  }));

  const emitters = (factories.features as { properties: Record<string, unknown> }[])
    .map((f) => ({
      name: String(locale === "ru" ? f.properties.name_ru : f.properties.name_kk),
    ...
    <PanelShell title={t.pollution.title}>
      ...
      <PanelItem>
        <ChartFrame
          ...
          subtitle={
            air.data?.live
              ? `${t.pollution.aqiLive} · ${
                  air.data.fetched_at ? new Date(air.data.fetched_at).toLocaleString(locale === "ru" ? "ru-RU" : "kk-KZ") : ""
                }`
              : t.common.offline
          }
      ...
      <PanelItem>
        <ChartFrame
          ...
          subtitle={locale === "ru" ? "0–100, выше — чище" : "0–100, жоғары — тазарақ"}
          note={locale === "ru" ? pollution.purity_note_ru : pollution.purity_note_kk}
          sourceId="model"
        >
        ...
        <ChartFrame title={t.pollution.factories} subtitle={t.pollution.emissions} sourceId="osm">
          <ResponsiveContainer width="100%" height={185}>
            <BarChart data={emitters} layout="vertical" margin={{ top: 0, right: 20, bottom: 0, left: 4 }} barCategoryGap={7}>
              ...
              <Tooltip {...tip} formatter={fmt((v) => `${v.toLocaleString("ru-RU")} т/год`)} />
